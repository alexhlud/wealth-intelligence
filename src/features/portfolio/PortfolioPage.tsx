...
export function HoldingsPage() {
  const { portfolio, accounts, positions } = usePortfolioData(); const [selected, setSelected] = useState<Set<string> | null>(null)
  ...
  return <section><header className="page-header"><div><p className="page-eyebrow">Portfolio</p><h1>Holdings, stated plainly.</h1><p>{portfolio.data?.name}</p></div><div className="holdings-header-actions"><p className="holdings-context">{countLabel}</p><Button className="button-primary" disabled title="Position entry is available in Phase 3b-2"><Plus size={16} /> Add position</Button></div></header><div className="holdings-summary"><div><p>Selected holdings value</p><strong>{formatUsd(total)}</strong>{unavailablePriceCount > 0 && <p className="incomplete-total-notice" role="status">{unavailablePriceCount} holding{unavailablePriceCount === 1 ? '' : 's'} excluded, price unavailable.</p>}</div><p className="day-change-unavailable">Day change unavailable<br /><span>Quotes arrive in Phase 4.</span></p><AccountSelection accounts={accountList} selected={selected} setSelected={persistSelected} /></div>{positions.isFetching && <p className="refetch-copy" role="status">Refreshing holdings…</p>}<div className="table-wrap"><table className="data-table"><thead><tr><th>Security</th><th>Shares</th><th>Average cost</th><th>Price</th><th>Day</th><th>Market value</th><th>Allocation</th></tr></thead><tbody>{visible.map((position) => { const price = HARDCODED_USD_PRICE_BY_SYMBOL[position.symbol]; const value = price ? marketValue(position.quantity, price) : null; const account = accountList.find(({ id }) => id === position.account_id); return <tr key={position.id}><td><strong>{position.security_name}</strong><small>{position.symbol} · {account?.name ?? 'Account unavailable'}</small></td><td>{position.quantity}</td><td>{formatUsd(position.average_cost)}</td><td>{price ? `${formatUsd(price)} fixed` : 'Unavailable'}</td><td><span className="unavailable-measurement">Unavailable<span className="sr-only">: quotes arrive in Phase 4</span></span></td><td>{value ? formatUsd(value) : 'Unavailable'}</td><td>{value ? percentageOf(value, total) ?? 'Unavailable' : 'Unavailable'}</td></tr>})}</tbody></table></div>{visible.length === 0 && <EmptyState>{selected.size ? 'No open positions belong to the selected accounts.' : 'Select at least one account to inspect its holdings.'}</EmptyState>}<p className="temporary-price">Temporary display source: QQQ $500.00 and VOO $600.00 fixed prices. Current quotes arrive in Phase 4.</p></section>
}
